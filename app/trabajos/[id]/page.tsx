...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-8"><div className="mx-auto grid max-w-6xl gap-6"><Link href="/trabajos" className="text-sm font-medium">← Trabajos</Link><header><p className="text-sm font-semibold uppercase tracking-widest text-emerald-700">{job.category.replace("_", " ")}</p><h1 className="text-3xl font-bold">{job.prism_number || job.title}</h1><p className="mt-2 text-slate-600">{job.title}{job.address ? ` · ${job.address}` : ""}</p><p className="mt-1 text-slate-600">Estado: {job.main_status.replaceAll("_", " ")}{job.incident ? ` · Incidencia: ${job.incident}` : ""}</p></header><JobDocuments jobId={job.id} originalPath={job.project_pdf_url} deliveredPath={job.delivered_pdf_path} deliveredStatus={getDeliveredPdfStatus(job, photos.map((photo) => photo.id))} jobStatus={job.main_status} canRegenerate /><OfficeJobActions jobId={job.id} status={job.main_status} assignment={assignment} options={options} /><section><h2 className="mb-3 text-lg font-semibold">Datos del trabajo</h2><JobForm job={job} /></section>{photos.length > 0 && <section><h2 className="text-lg font-semibold">Evidencias</h2><p className="mt-1 text-sm text-slate-600">{photos.length} fotografía(s) confirmada(s)</p><JobEvidenceList photos={photos} /></section>}<Timeline entries={history} /></div></main>;
}
