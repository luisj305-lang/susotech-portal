...
async function TechnicianDetail({ id }: { id: string }) {
  ...
  return <main className="min-h-screen bg-black px-4 py-6 text-white"><div className="mx-auto grid max-w-xl gap-5"><Link href="/trabajos" className="text-sm font-medium text-white">← Mis trabajos</Link><header><p className="text-sm font-semibold uppercase tracking-widest text-white">{job.category.replace("_", " ")}</p><h1 className="mt-1 text-3xl font-bold">{job.title}</h1><p className="mt-2 text-white">{job.address || job.location || "Ubicación no indicada"}</p></header>
    <section className="rounded-2xl bg-black p-5 text-white shadow-lg"><h2 className="text-xl font-bold">Instrucciones</h2><dl className="mt-3 grid gap-3"><div><dt className="font-semibold">Descripción</dt><dd>{job.description || "Sin descripción"}</dd></div><div><dt className="font-semibold">Instrucciones especiales</dt><dd>{job.special_instructions || "Ninguna"}</dd></div><div><dt className="font-semibold">Material requerido</dt><dd>{job.required_material || "No indicado"}</dd></div></dl>{job.project_map_url && <a href={job.project_map_url} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-12 items-center font-bold text-white underline">Abrir mapa del proyecto</a>}</section>
    <JobDocuments jobId={job.id} originalPath={job.project_pdf_url} deliveredPath={job.delivered_pdf_path} deliveredStatus={getDeliveredPdfStatus(job, photos.map((photo) => photo.id))} jobStatus={job.main_status} />
    <TechnicianActions jobId={job.id} status={job.main_status} incident={job.incident} /><CodeInput jobId={job.id} enabled={job.main_status === "en_progreso"} />
...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!detail) notFound();
  const { job, assignment, history, options, photos } = detail;
  return <main className="min-h-screen bg-black px-4 py-8 text-white sm:px-8"><div className="mx-auto grid max-w-6xl gap-6"><Link href="/trabajos" className="text-sm font-medium text-white">← Trabajos</Link><header><p className="text-sm font-semibold uppercase tracking-widest text-white">{job.category.replace("_", " ")}</p><h1 className="text-3xl font-bold">{job.prism_number || job.title}</h1><p className="mt-2 text-white">{job.title}{job.address ? ` · ${job.address}` : ""}</p><p className="mt-1 text-white">Estado: {job.main_status.replaceAll("_", " ")}{job.incident ? ` · Incidencia: ${job.incident}` : ""}</p></header><JobDocuments jobId={job.id} originalPath={job.project_pdf_url} deliveredPath={job.delivered_pdf_path} deliveredStatus={getDeliveredPdfStatus(job, photos.map((photo) => photo.id))} jobStatus={job.main_status} canRegenerate /><OfficeJobActions jobId={job.id} status={job.main_status} assignment={assignment} options={options} /><section className="rounded-2xl border border-white bg-black p-5"><h2 className="mb-3 text-lg font-semibold">Datos del trabajo</h2><div className="text-white"><JobForm job={job} /></div></section>{photos.length > 0 && <section className="rounded-2xl border border-white bg-black p-5"><h2 className="text-lg font-semibold">Evidencias</h2><p className="mt-1 text-sm text-white">{photos.length} fotografía(s) confirmada(s)</p><JobEvidenceList photos={photos} /></section>}<Timeline entries={history} /></div></main>;
}
