-- Fix PDF text-note validation after arrow fields were added.
-- jsonb_object_keys returns a set whose output column must be named explicitly;
-- otherwise `value` is resolved as a nonexistent column when a note is saved.
...
begin
  ...
  for item in select value from jsonb_array_elements(p_text_notes) loop
    if jsonb_typeof(item) <> 'object'
      ...
      or (select count(*)
        from jsonb_object_keys(item) as note_keys(note_key)
        where note_key not in (
          'page','sourceDocumentId','sourcePage','text','x','y',
