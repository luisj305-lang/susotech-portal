-- Correct the pending backfill contract: helpers may participate in an
-- allocation but can never be inferred as the primary deliverer.
delete from public.job_delivery_allocation_versions v
using public.job_delivery_financial_allocations a
where a.allocation_version_id = v.id
  and v.version = 1
  and v.reason = 'Unambiguous legacy delivery backfill'
  and v.idempotency_key = md5('unambiguous-delivery-allocation-backfill-v1:' || v.delivery_id::text)::uuid
  and a.worker_specialty_snapshot = 'ayudante';
