-- Office staff (admin + supervisor) can list the active jobs assigned to a
-- specific technician (directly or through an active crew). Mirrors the
-- can_mutate_job assignment logic.
...
begin
  ...
  return query
  select j.id, j.prism_number, j.title, j.address, j.main_status::text, j.deadline_date, j.updated_at
  from public.jobs j
