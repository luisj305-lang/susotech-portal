-- Include archived jobs in the technician assigned-jobs list and expose
-- archived_at so the UI can flag them. The return type changes, so the
-- function is dropped first (create or replace cannot alter OUT columns).
