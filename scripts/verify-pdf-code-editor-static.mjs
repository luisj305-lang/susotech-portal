...
assert.match(editor,/Volver a editar el PDF/u); assert.match(technicianActions,/router\.push\(`\/trabajos\/\$\{jobId\}\/entregar`\)/u);
assert.doesNotMatch(editor+technicianActions,/CustomEvent|job-pdf-draft-save-request/u);
assert.doesNotMatch(detail,/PdfCodeEditor/u); assert.match(deliveryPage,/isOperationalFieldWorker\(profile\)/u); assert.match(deliveryPage,/\["asignado", "en_revision"\]\.includes/u);
...
assert.match(editor,/fontSize: `\$\{fittedFontSize\}cqw`/u);
assert.match(compositor,/color: rgb\(1, 1, 1\),\s*borderColor: color,\s*borderWidth: 2/u);
assert.match(compositor,/codeFont\.widthOfTextAtSize\(text, fittedTextSize\)/u);
assert.match(compositor,/font: codeFont,\s*color: rgb\(0, 0, 0\)/u);
assert.match(editor,/Cantidad del código/u); assert.match(editor,/arrowTipX/u); assert.match(sql,/p_source_document_ids/u);
assert.match(editor,/Borrador guardado/u); assert.match(editor,/saveInFlight/u); assert.match(editor,/persistStableDraft/u);
