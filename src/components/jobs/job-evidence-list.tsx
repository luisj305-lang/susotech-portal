  ...
  return <>
    <ul className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {photos.map((photo, index) => {
        ...
        return <li key={photo.id} className="overflow-hidden rounded-lg border border-black bg-white text-black">
          <button
            type="button"
          ...
          >
            {signedUrl
              ? <Image
                  ...
                  unoptimized
                />
              : <span className="flex h-full items-center justify-center text-sm text-black/70">Cargando foto…</span>}
          </button>
          <div className="p-3">
            <strong>Foto {photos.length - index}</strong>
            <p className="text-sm text-black">{new Intl.DateTimeFormat("es-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date(photo.created_at))}</p>
            {photo.comment && <p className="mt-2 text-sm">{photo.comment}</p>}
            {canDelete && <button type="button" disabled={pending} onClick={() => setSelectedPhoto(photo)} className="mt-3 min-h-11 rounded-lg border border-black px-3 font-semibold disabled:opacity-60">Eliminar fotografía</button>}
          </div>
        </li>;
      })}
    </ul>
    {selectedPhoto && <div role="dialog" aria-modal="true" aria-labelledby="delete-photo-title" className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4"><div className="w-full max-w-md rounded-2xl border border-black bg-white p-5 text-black shadow-2xl"><h2 id="delete-photo-title" className="text-xl font-bold">Eliminar fotografía</h2>{evidenceUrls[selectedPhoto.id] && <div className="relative mt-4 aspect-[4/3] overflow-hidden rounded-lg border border-black"><Image src={evidenceUrls[selectedPhoto.id]} alt="Fotografía que se eliminará" fill className="object-cover" unoptimized /></div>}<dl className="mt-4 grid gap-2 text-sm"><div><dt className="font-bold">Técnico</dt><dd>{selectedPhoto.uploader_name || "Usuario registrado"}</dd></div><div><dt className="font-bold">Fecha</dt><dd>{new Intl.DateTimeFormat("es-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date(selectedPhoto.created_at))}</dd></div></dl><p className="mt-4 border border-black p-3 font-semibold">Esta acción eliminará la fotografía de la evidencia del trabajo y puede dejar desactualizado el PDF entregado.</p><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" disabled={pending} onClick={() => setSelectedPhoto(null)} className="min-h-12 border border-black font-bold">Cancelar</button><button type="button" disabled={pending} onClick={() => startTransition(async () => { const result = await deleteJobPhoto({ jobId: selectedPhoto.job_id, photoId: selectedPhoto.id }); setMessage(result.message); if (result.success) { setSelectedPhoto(null); router.refresh(); } })} className="min-h-12 bg-black font-bold text-white disabled:opacity-60">Sí, eliminar</button></div></div></div>}
    <p role="status" aria-live="polite" className="mt-3 text-sm text-black">{pending ? "Abriendo evidencia…" : message}</p>
  </>;
}
