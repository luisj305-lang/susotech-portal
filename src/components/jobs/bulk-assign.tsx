...
export function BulkAssign({ jobs, options }: { jobs: ImportedJob[]; options: AssigneeOption[] }) {
  ...
  return <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Asignar trabajos importados</h2><p className="mt-1 text-sm text-slate-600">Selecciona uno para asignación individual o varios para asignación masiva.</p>
    ...
    <form action={assign} className="flex flex-col gap-3 sm:flex-row"><label className="grid flex-1 gap-1 text-sm font-medium">Técnico o crew<select name="assignee" required className="rounded-lg border p-3"><option value="">Selecciona una opción</option>{options.map((option) => <option key={`${option.type}:${option.id}`} value={`${option.type}:${option.id}`}>{option.type === "crew" ? "Crew" : "Técnico"}: {option.label}</option>)}</select></label><button disabled={pending || !selected.length || !options.length} className="self-end rounded-lg bg-slate-900 px-5 py-3 font-semibold text-white disabled:opacity-60">{pending ? "Asignando…" : `Asignar ${selected.length || ""}`}</button></form>
