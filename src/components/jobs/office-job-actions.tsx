...
export function OfficeJobActions({ jobId, status, assignment, options, canArchive, archived }: { jobId: string; status: JobStatus; assignment: JobAssignment | null; options: AssigneeOption[]; canArchive?: boolean; archived?: boolean }) {
  ...
  return <section className="grid gap-4 rounded-2xl border border-black bg-white p-5 shadow-sm">
    <h2 className="text-lg font-semibold">Asignación y estado</h2>
    {assignment?.assignee_type === "crew" && <p className="rounded-lg border border-black p-3 text-sm">Este trabajo conserva una asignación histórica por crew. Selecciona un responsable individual para reemplazarla.</p>}
    <form action={(data) => { const [, assigneeId] = String(data.get("assignee")).split(":"); void run(() => assignJob({ jobId, assigneeType: "technician", assigneeId })); }} className="flex flex-col gap-3 sm:flex-row">
      ...
      <button disabled={pending || !options.length} className="self-end rounded-lg border border-black px-5 py-3 font-semibold disabled:opacity-60">Guardar asignación</button>
      {assignment && status === "asignado" && <button type="button" disabled={pending} onClick={() => run(() => unassignJob({ jobId }))} className="self-end rounded-lg border border-black px-5 py-3 font-semibold disabled:opacity-60">Quitar asignación</button>}
    </form>
    ...
    {canArchive && (archived ? <button type="button" disabled={pending} onClick={() => run(() => setJobArchived({ jobId, archived: false }))} className="w-fit rounded-lg border border-black px-5 py-3 font-semibold disabled:opacity-60">Restaurar trabajo</button> : <form action={(data) => void run(() => setJobArchived({ jobId, archived: true, reasonCode: String(data.get("archiveReasonCode") ?? ""), notes: String(data.get("archiveNotes") ?? "") }))} className="grid gap-3 border-t border-black pt-4"><label className="grid gap-1 text-sm font-medium">Motivo para retirar del dashboard<select name="archiveReasonCode" required defaultValue="" className="rounded-lg border border-black bg-white p-3 text-black"><option value="" disabled>Selecciona un motivo</option><option value="duplicate_job">Trabajo duplicado</option><option value="cancelled_by_client_or_office">Cancelado por el cliente o la oficina</option><option value="incorrect_address_or_data">Dirección o datos incorrectos</option><option value="no_access_or_blocked_conditions">Sin acceso o condiciones que impiden realizarlo</option><option value="out_of_scope">Fuera de alcance o no corresponde a Susotech</option></select></label><label className="grid gap-1 text-sm font-medium">Observaciones (opcional)<textarea name="archiveNotes" maxLength={2000} rows={3} className="rounded-lg border border-black bg-white p-3 text-black" /></label><button disabled={pending} className="w-fit rounded-lg border border-black px-5 py-3 font-semibold disabled:opacity-60">Archivar trabajo</button></form>)}
