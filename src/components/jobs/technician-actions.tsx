...
export function TechnicianActions({ jobId, status, incident }: { jobId: string; status: JobStatus; incident: IncidentType | null }) {
  ...
  return <section className="grid gap-4 rounded-2xl bg-white p-5 text-black shadow-lg"><h2 className="text-xl font-bold">Acciones</h2>{advance && <button type="button" disabled={pending} onClick={() => run(() => transitionJob({ jobId, newStatus: advance.status }))} className="min-h-14 rounded-xl bg-black px-5 text-lg font-bold text-white disabled:opacity-60">{advance.label}</button>}{["en_progreso", "enviado_revision"].includes(status) && <button type="button" disabled={pending} onClick={deliver} className="min-h-14 rounded-xl bg-black px-5 text-lg font-bold text-white disabled:opacity-60">Abrir o continuar la entrega</button>}
