...
export function TechnicianActions({ jobId, status, incident }: { jobId: string; status: JobStatus; incident: IncidentType | null }) {
  ...
  return <section className="grid gap-4 rounded-2xl bg-white p-5 text-slate-950 shadow-lg"><h2 className="text-xl font-bold">Acciones</h2>{advance && <button type="button" disabled={pending} onClick={() => run(() => transitionJob({ jobId, newStatus: advance.status }))} className="min-h-14 rounded-xl bg-emerald-700 px-5 text-lg font-bold text-white disabled:opacity-60">{advance.label}</button>}{status === "en_progreso" && <button type="button" disabled={pending} onClick={deliver} className="min-h-14 rounded-xl bg-emerald-700 px-5 text-lg font-bold text-white disabled:opacity-60">Entregar trabajo</button>}
