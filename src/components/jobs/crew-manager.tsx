...
export function CrewManager({ crews, technicians, canManage }: { crews: CrewOfficeDto[]; technicians: TechnicianDirectoryOption[]; canManage: boolean }) {
  ...
  return <div className="grid gap-6">
    ...
    </form>{status("create")}</section> : <section className="rounded-2xl border bg-white p-4 text-sm"><strong>Modo consulta.</strong> Solo los administradores pueden modificar equipos y membresías.</section>}

    {!crews.length ? <section className="rounded-2xl border border-dashed bg-white p-10 text-center"><h2 className="font-semibold">No hay equipos</h2><p className="mt-2 text-sm text-slate-600">{canManage ? "Crea el primer equipo usando el formulario anterior." : "No hay equipos disponibles para consultar."}</p></section> : <div className="grid gap-5 lg:grid-cols-2">{crews.map((crew) => {
      const candidates = availableCrewMembers(crew, technicians);
      return <article key={crew.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{crew.name}</h2><p className="text-sm text-slate-600">Líder técnico: {crew.lead_label}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${crew.is_active ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{crew.is_active ? "Activo" : "Inactivo"}</span></div>
