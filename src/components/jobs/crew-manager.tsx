...
export function CrewManager({ crews, technicians, canManage }: { crews: CrewOfficeDto[]; technicians: TechnicianDirectoryOption[]; canManage: boolean }) {
  ...
  const status = (scope: string) => feedback?.scope === scope && <p role="status" className={`mt-3 text-sm font-medium ${feedback.error ? "text-white" : "text-white"}`}>{feedback.message}</p>;

  return <div className="grid gap-6">
    {canManage ? <section className="rounded-2xl border bg-black p-5 shadow-sm"><h2 className="text-lg font-bold">Crear equipo</h2><form className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); run("create", () => createCrew({ name: String(form.get("name")), leadTechnicianId: String(form.get("lead")) })); }}>
      ...
      <label className="grid gap-1 text-sm font-medium">Líder técnico<select name="lead" required defaultValue="" className="rounded-lg border p-3"><option value="" disabled>Selecciona un técnico</option>{technicians.map((tech) => <option key={tech.id} value={tech.id}>{tech.label}</option>)}</select></label>
      <button disabled={pending || !technicians.length} className="self-end rounded-lg bg-black px-5 py-3 font-semibold text-white disabled:opacity-50">Crear equipo</button>
    </form>{status("create")}</section> : <section className="rounded-2xl border border-white bg-black p-4 text-sm text-white"><strong>Modo consulta.</strong> Solo los administradores pueden modificar equipos y membresías.</section>}

    {!crews.length ? <section className="rounded-2xl border border-dashed bg-black p-10 text-center"><h2 className="font-semibold">No hay equipos</h2><p className="mt-2 text-sm text-white">{canManage ? "Crea el primer equipo usando el formulario anterior." : "No hay equipos disponibles para consultar."}</p></section> : <div className="grid gap-5 lg:grid-cols-2">{crews.map((crew) => {
      const candidates = availableCrewMembers(crew, technicians);
      return <article key={crew.id} className="rounded-2xl border bg-black p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{crew.name}</h2><p className="text-sm text-white">Líder técnico: {crew.lead_label}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${crew.is_active ? "bg-black text-white" : "bg-black text-white"}`}>{crew.is_active ? "Activo" : "Inactivo"}</span></div>
        {canManage && <form className="mt-5 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); run(crew.id, () => updateCrew({ crewId: crew.id, name: String(form.get("name")), leadTechnicianId: String(form.get("lead")) })); }}>
          ...
          <label className="grid gap-1 text-sm font-medium">Líder técnico<select name="lead" required defaultValue={crew.lead_technician_id} className="rounded-lg border p-3">{!technicians.some((tech) => tech.id === crew.lead_technician_id) && <option value={crew.lead_technician_id}>Líder no disponible</option>}{technicians.map((tech) => <option key={tech.id} value={tech.id}>{tech.label}</option>)}</select></label>
          <button disabled={pending} className="rounded-lg border px-4 py-3 font-semibold disabled:opacity-50">Guardar cambios</button>
        </form>}
        <div className="mt-5"><h3 className="font-semibold">Técnicos miembros</h3>{crew.members.length ? <ul className="mt-2 grid gap-2">{crew.members.map((member) => <li key={member.id} className="flex items-center justify-between gap-3 rounded-lg bg-black p-3"><span>{member.label}{member.id === crew.lead_technician_id ? " · Líder" : ""}</span>{canManage && <button type="button" disabled={pending || !canRemoveCrewMember(crew, member.id)} onClick={() => window.confirm(`¿Quitar a ${member.label} del equipo?`) && run(crew.id, () => removeCrewMember({ crewId: crew.id, technicianId: member.id }))} className="rounded-md border px-3 py-2 text-sm font-semibold disabled:opacity-40">Quitar</button>}</li>)}</ul> : <p className="mt-2 text-sm text-white">Sin integrantes adicionales.</p>}
        ...
        {canManage && <button type="button" disabled={pending} onClick={() => window.confirm(`¿${crew.is_active ? "Desactivar" : "Activar"} ${crew.name}?`) && run(crew.id, () => setCrewActive({ crewId: crew.id, active: !crew.is_active }))} className="mt-5 rounded-lg bg-black px-4 py-3 font-semibold text-white disabled:opacity-50">{crew.is_active ? "Desactivar equipo" : "Activar equipo"}</button>}
        {status(crew.id)}
      </article>;
    })}</div>}
  </div>;
