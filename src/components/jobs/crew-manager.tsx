...
export function CrewManager({ crews, technicians, canManage }: { crews: CrewOfficeDto[]; technicians: TechnicianDirectoryOption[]; canManage: boolean }) {
  ...
  const status = (scope: string) => feedback?.scope === scope && <p role="status" className={`mt-3 text-sm font-medium ${feedback.error ? "text-black" : "text-black"}`}>{feedback.message}</p>;

  return <div className="grid gap-6">
    {canManage ? <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-lg font-bold">Crear equipo</h2><form className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); run("create", () => createCrew({ name: String(form.get("name")), leadTechnicianId: String(form.get("lead")) })); }}>
    ...
    </form>{status("create")}</section> : <section className="rounded-2xl border border-black bg-white p-4 text-sm text-black"><strong>Modo consulta.</strong> Solo los administradores pueden modificar equipos y membresías.</section>}

    {!crews.length ? <section className="rounded-2xl border border-dashed bg-white p-10 text-center"><h2 className="font-semibold">No hay equipos</h2><p className="mt-2 text-sm text-black">{canManage ? "Crea el primer equipo usando el formulario anterior." : "No hay equipos disponibles para consultar."}</p></section> : <div className="grid gap-5 lg:grid-cols-2">{crews.map((crew) => {
      const candidates = availableCrewMembers(crew, technicians);
      return <article key={crew.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{crew.name}</h2><p className="text-sm text-black">Líder técnico: {crew.lead_label}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${crew.is_active ? "bg-white text-black" : "bg-white text-black"}`}>{crew.is_active ? "Activo" : "Inactivo"}</span></div>
        ...
        <div className="mt-5"><h3 className="font-semibold">Técnicos miembros</h3>{crew.members.length ? <ul className="mt-2 grid gap-2">{crew.members.map((member) => <li key={member.id} className="flex items-center justify-between gap-3 rounded-lg bg-white p-3"><span>{member.label}{member.id === crew.lead_technician_id ? " · Líder" : ""}</span>{canManage && <button type="button" disabled={pending || !canRemoveCrewMember(crew, member.id)} onClick={() => window.confirm(`¿Quitar a ${member.label} del equipo?`) && run(crew.id, () => removeCrewMember({ crewId: crew.id, technicianId: member.id }))} className="rounded-md border px-3 py-2 text-sm font-semibold disabled:opacity-40">Quitar</button>}</li>)}</ul> : <p className="mt-2 text-sm text-black">Sin integrantes adicionales.</p>}
