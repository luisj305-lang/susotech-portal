...
export function PhotoUpload({ jobId }: { jobId: string }) {
  ...
  return <section className="grid gap-6 rounded-2xl bg-black p-5 text-white shadow-lg"><div><h2 className="text-xl font-bold">Evidencia fotográfica</h2><form action={upload} className="mt-4 grid gap-3"><label className="grid gap-1 font-semibold">Foto<input ref={input} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" required onChange={(event) => setPendingFile(event.target.files?.[0]?.name ?? "")} className="min-h-12 rounded-xl border p-3" /></label><label className="grid gap-1 font-semibold">Tipo<select name="photoType" className="min-h-12 rounded-xl border p-3"><option value="before">Antes</option><option value="after">Después</option><option value="evidence">Evidencia</option></select></label><label className="grid gap-1 font-semibold">Comentario de esta foto <span className="font-normal text-white">(opcional)</span><textarea name="photoComment" rows={2} maxLength={2000} className="rounded-xl border p-3" /></label><button disabled={pending} className="min-h-14 rounded-xl bg-black px-5 text-lg font-bold text-white disabled:opacity-60">{pending ? "Subiendo…" : "Subir foto"}</button></form></div><form action={comment} className="grid gap-3 border-t pt-5"><label className="grid gap-1 font-semibold">Comentario general del trabajo<textarea name="comment" required rows={3} className="rounded-xl border p-3" /></label><button disabled={pending} className="min-h-14 rounded-xl border-2 border-white px-5 text-lg font-bold">Guardar comentario general</button></form><UploadFeedback message={message} pendingFile={pendingFile} /></section>;
}
