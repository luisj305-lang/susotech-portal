...
export function ArchivedJobDeleteButton({
  ...
  return <div className="mt-3 grid gap-2">
    <button
      ...
      disabled={pending}
      onClick={remove}
      className="min-h-11 w-fit rounded-lg border border-white px-5 font-bold text-white disabled:opacity-60"
    >
      {pending ? "Eliminando…" : "Eliminar permanentemente"}
    </button>
    <p role="status" aria-live="polite" className="text-sm text-white">{message}</p>
  </div>;
}
...
export function RetryJobDeletionCleanupButton() {
  ...
  return <div className="grid gap-2">
    <button
      ...
      className="rounded-lg border border-white px-5 py-3 font-semibold disabled:opacity-60"
    >
