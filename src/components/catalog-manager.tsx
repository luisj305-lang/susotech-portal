...
export function CatalogManager({ canManage, initialItems, categories, rates }: {
  ...
  return <main className="min-h-screen p-4 sm:p-8">
    ...
    {canManage && <form action={submitItem} className="mt-6 grid max-w-4xl gap-3 border border-current p-4 sm:grid-cols-2">
      ...
      <label className="grid gap-1">Unidad<select name="unit" defaultValue={editing?.unit ?? "fixed"} className="min-h-11 border border-current bg-black p-2"><option value="fixed">Unidad</option><option value="foot">Pie</option><option value="hour">Hora</option><option value="event">Evento</option></select></label>
    ...
    {rateTarget && <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4"><form action={submitRate} className="grid w-full max-w-md gap-3 border border-white bg-black p-5 text-white"><h2 className="text-xl font-bold">{rateTarget.item.code} · {rateTarget.category.name}</h2><p>{rateTarget.item.description}</p><label className="grid gap-1">Precio unitario<input name="unitPrice" type="number" min="0" step="0.001" defaultValue={rateTarget.price} required className="min-h-11 border border-white bg-black p-2" /></label><label className="grid gap-1">Vigente desde<input name="effectiveFrom" type="date" defaultValue={today()} required className="min-h-11 border border-white bg-black p-2" /></label><label className="flex gap-2"><input name="active" type="checkbox" defaultChecked />Activa</label><div className="flex gap-2"><button disabled={pending} className="min-h-11 bg-white px-4 font-bold text-black">Guardar tarifa</button><button type="button" onClick={() => setRateTarget(null)} className="min-h-11 border border-white px-4">Cancelar</button></div></form></div>}
  </main>;
}
