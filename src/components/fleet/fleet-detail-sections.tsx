...
function ShiftAssociationsCard({ detail }: { detail: FleetVehicleDetail }) {
  return <Card><CardHeader><CardTitle>Asociación de jornadas</CardTitle><p className="text-sm text-ink-soft">Audite el camión capturado al iniciar cada jornada y corríjalo cuando sea necesario.</p></CardHeader><CardContent className="grid gap-3">{detail.shiftAssociations.length ? detail.shiftAssociations.map((shift) => <details key={shift.id} className="rounded-xl border border-line p-4"><summary className="cursor-pointer font-semibold">{shift.technician_name} · {new Date(shift.started_at).toLocaleString("es-US")} · {shift.vehicle_unit_number ?? "Sin camión"}</summary><FleetActionForm action={setFleetShiftVehicleAction} submitLabel="Guardar asociación" className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]"><Hidden name="shift_id" value={shift.id} /><Hidden name="route_vehicle_id" value={detail.vehicle.id} /><label className="grid gap-1 text-sm font-medium text-ink-soft">Camión<select name="target_vehicle_id" defaultValue={shift.vehicle_id ?? ""} className={fieldClass}><option value="">Sin camión</option>{detail.vehicleOptions.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.unit_number}</option>)}</select></label></FleetActionForm></details>) : <Empty>No hay jornadas recientes.</Empty>}</CardContent></Card>;
}
...
export function FleetDetailSections({ detail, activeTab }: { detail: FleetVehicleDetail; activeTab: FleetTab }) {
  return <div className="space-y-6">
    ...
    {activeTab === "gastos" ? <><ExpensesSection detail={detail} /><ShiftAssociationsCard detail={detail} /></> : null}
