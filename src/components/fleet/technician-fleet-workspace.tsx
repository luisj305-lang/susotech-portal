...
function OdometerForm({ vehicle }: { vehicle: TechnicianFleetVehicle }) {
  return (
    <FleetActionForm action={submitMyFleetOdometerAction} submitLabel="Registrar millaje" resetOnSuccess className="grid gap-3">
...
function IncidentForm({ vehicle }: { vehicle: TechnicianFleetVehicle }) {
  return (
    <FleetActionForm action={reportMyFleetIncidentAction} submitLabel="Enviar incidencia" resetOnSuccess className="grid gap-3 sm:grid-cols-2">
      <input type="hidden" name="vehicle_id" value={vehicle.vehicle.id} />
...
function VehicleCard({ vehicle, heading }: { vehicle: TechnicianFleetVehicle; heading: string }) {
  ...
  return (
    <Card>
      <CardHeader>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-muted">{heading}</p>
        <CardTitle>{master.unit_number} · {master.make} {master.model}</CardTitle>
        <p className="text-sm text-ink-soft">{vehicleStatusLabels[master.status]}{master.model_year ? ` · ${master.model_year}` : ""}{master.license_plate ? ` · placa ${master.license_plate}` : ""}</p>
      </CardHeader>
      <CardContent className="grid gap-6">
        <dl className="grid gap-3 text-sm sm:grid-cols-2">
          <div><dt className="text-ink-muted">Millaje actual</dt><dd className="font-semibold text-ink">{Number(master.current_odometer_miles).toLocaleString("en-US")} mi</dd></div>
          <div><dt className="text-ink-muted">VIN</dt><dd className="font-semibold text-ink">{master.vin ?? "No registrado"}</dd></div>
          <div><dt className="text-ink-muted">Asignación desde</dt><dd className="font-semibold text-ink">{vehicle.assignment.starts_on}</dd></div>
          <div><dt className="text-ink-muted">Finaliza</dt><dd className="font-semibold text-ink">{vehicle.assignment.ends_on ?? "Sin fecha"}</dd></div>
        </dl>
        ...
        <details className="rounded-xl border border-line p-4" open={vehicle.assignment.assignment_role === "primary"}>
          <summary className="cursor-pointer font-semibold">Registrar millaje</summary>
          <div className="mt-4"><OdometerForm vehicle={vehicle} /></div>
        </details>
        <details className="rounded-xl border border-line p-4">
          <summary className="cursor-pointer font-semibold">Reportar incidencia</summary>
          <div className="mt-4"><IncidentForm vehicle={vehicle} /></div>
        </details>
        <div className="grid gap-4 lg:grid-cols-3">
          <section><h3 className="font-semibold">Lecturas recientes</h3><div className="mt-2 grid gap-2 text-sm">{vehicle.recentOdometer.length ? vehicle.recentOdometer.map((reading) => <p key={reading.id} className="rounded-lg bg-surface-muted p-3">{reading.recorded_on}: <strong>{Number(reading.reading_miles).toLocaleString("en-US")} mi</strong></p>) : <p className="text-ink-muted">Sin lecturas.</p>}</div></section>
          <section><h3 className="font-semibold">Incidencias recientes</h3><div className="mt-2 grid gap-2 text-sm">{vehicle.recentIncidents.length ? vehicle.recentIncidents.map((incident) => <p key={incident.id} className="rounded-lg bg-surface-muted p-3"><strong>{incident.title}</strong><br />{incidentStatusLabels[incident.status]} · {severityLabels[incident.severity]}</p>) : <p className="text-ink-muted">Sin incidencias.</p>}</div></section>
          <section><h3 className="font-semibold">Documentos recientes</h3><div className="mt-2 grid gap-2 text-sm">{vehicle.recentDocuments.length ? vehicle.recentDocuments.map((document) => document.signed_url ? <a key={document.id} href={document.signed_url} target="_blank" rel="noopener noreferrer" className="rounded-lg bg-surface-muted p-3 font-semibold underline">{documentLabels[document.document_type]} · {document.title}</a> : <p key={document.id} className="rounded-lg bg-surface-muted p-3">{document.title} · enlace no disponible</p>) : <p className="text-ink-muted">Sin documentos.</p>}</div></section>
        </div>
      </CardContent>
    </Card>
  );
...
export function TechnicianFleetWorkspace({ workspace }: { workspace: TechnicianFleetWorkspaceData }) {
  ...
  return (
    <div className="grid gap-6">
      <Alert variant={weeklyVariant}>{weeklyMessage}</Alert>
      {workspace.primary ? <VehicleCard vehicle={workspace.primary} heading="Camión principal" /> : <Card><CardHeader><CardTitle>Sin camión asignado</CardTitle></CardHeader><CardContent><p className="text-sm text-ink-soft">La oficina todavía no registró un camión principal para tu perfil.</p></CardContent></Card>}
      {workspace.backups.length ? <section className="grid gap-4"><h2 className="text-xl font-bold text-ink">Camiones suplentes</h2>{workspace.backups.map((vehicle) => <VehicleCard key={vehicle.assignment.id} vehicle={vehicle} heading="Asignación suplente" />)}</section> : null}
    </div>
  );
}
